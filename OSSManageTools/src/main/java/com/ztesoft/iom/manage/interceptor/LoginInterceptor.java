package com.ztesoft.iom.manage.interceptor;
...
public class LoginInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        // 判断session中是否有user属性，有则继续用户操作
        if(request.getSession().getAttribute("ossLoginUser") != null) {
            return true;
        } else {
            log.debug("当前会话ID[" + request.getSession().getId() + "]没有登录信息，需要重新登录……");
        }
    ...
    public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) {
        ...
        log.debug("会话ID[" + request.getSession().getId() + "]请求访问[" + request.getRequestURI() + "]成功……");
    }
}
